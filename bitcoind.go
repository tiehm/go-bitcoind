...
func New(host string, port int, user, passwd string, timeoutParam ...int) (*Bitcoind, error) {
	...
	rpcClient, err := newClient(host, port, user, passwd, timeout)
	if err != nil {
		return nil, err
	}
...
func (b *Bitcoind) GetRawTransaction(txId string, verbose bool, block string) (rawTx interface{}, err error) {
	intVerbose := 0
	if verbose {
		intVerbose = 1
	}
	r, err := b.client.call("getrawtransaction", []interface{}{txId, intVerbose, block})
	if err = handleError(err, &r); err != nil {
		return
	}
	if !verbose {
...
func (b *Bitcoind) SendRawTransaction(hexString string) (hex string, err error) {
	r, err := b.client.call("sendrawtransaction", []string{hexString})
	if err = handleError(err, &r); err != nil {
		return
	}
